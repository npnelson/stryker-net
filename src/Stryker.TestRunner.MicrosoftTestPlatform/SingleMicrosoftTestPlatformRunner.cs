...
public class SingleMicrosoftTestPlatformRunner : IDisposable
{
    ...
    /// <summary>
    /// Path of the mutant-id control file this runner shares with its test hosts. Exposed for unit tests.
    /// </summary>
        ...
        IStrykerOptions? options = null)
    {
        ...
        // Both names embed the process id plus a per-instance nonce. Runner ids restart at 0 in
        // every Stryker process, so id-only names are shared between concurrent runs: their
        // runners would steer each other's test hosts through the mutant-id file and clobber each
        // other's coverage files, and a run could read a stale coverage file left behind by a
        // crashed earlier run (coverage files are only deleted once their path has been assigned).
        // The nonce covers what the process id alone does not (pid reuse, several runner instances
        // with the same id in one process).
