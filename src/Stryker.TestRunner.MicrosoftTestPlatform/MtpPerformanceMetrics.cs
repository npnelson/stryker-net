...
internal sealed class MtpPhasePerformanceMetrics
{
    ...
    {
        var firstRequest = Interlocked.Read(ref _firstLeaseRequestTimestamp);
        var lastCompletion = Interlocked.Read(ref _lastLeaseCompletionTimestamp);
        var elapsed = firstRequest == long.MaxValue || lastCompletion < firstRequest
            ? TimeSpan.Zero
            : Stopwatch.GetElapsedTime(firstRequest, lastCompletion);
    ...
    double RunnerUtilizationPercent)
{
    ...
    public static MtpPerformanceSummary Create(TimeSpan runnerBusy, int runnerCount, TimeSpan elapsed)
    {
