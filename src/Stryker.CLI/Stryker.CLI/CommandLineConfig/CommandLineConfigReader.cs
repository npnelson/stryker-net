...
public class CommandLineConfigReader
{
    ...
    public void RegisterInitCommand(CommandLineApplication app, IFileSystem fileSystem, IStrykerInputs inputs, string[] args) =>
        app.Command("init", initCommandApp =>
        {
            ...
            initCommandApp.OnExecute(() =>
            {
                ...
                _ = fileSystem.Directory.CreateDirectory(Path.GetDirectoryName(configFilePath));
    ...
    {
        var commands = app.Parse(args);
        return commands.SelectedCommand.Options.SingleOrDefault(o => o.LongName == _skipVersionCheckInput.ArgumentName);
    }
    ...
    private void PrepareCliOptions(IStrykerInputs inputs)
    {
        ...
        AddCliInput(inputs.CoverageCohortSizeInput, "coverage-cohort-size", null, argumentHint: "number", category: InputCategory.Misc);
