...
| **C2** | **A host holds two or more mutated assemblies** — the same precondition as C1, so equally ordinary. One scalar ack serves N copies: the first ack releases the read, so the runner cannot know whether every copy flushed, and `Dubious` never fires when it matters. | **E2E** · ack [`365`](https://github.com/stryker-mutator/stryker-net/blob/fbf2ed618ff81c4643feb7a2d7263ba1127cb3e4/src/Stryker.Core/Stryker.Core/InjectedHelpers/MutantControl.cs#L365) · wait [`408-443`](https://github.com/stryker-mutator/stryker-net/blob/fbf2ed618ff81c4643feb7a2d7263ba1127cb3e4/src/Stryker.TestRunner.MicrosoftTestPlatform/SingleMicrosoftTestPlatformRunner.cs#L408-L443) · three identical runs give three verdict sets — §2 | untracked |
...
### End to end, on the repo's own fixture — C1 and C2
...
[`npnelson@835a096d`](https://github.com/npnelson/stryker-net/commit/835a096d606186d01d456252ddfc56e10ea9d398)
adds exactly that test, plus the `ProjectReference` and a coverage-off config. No product changes, and it
cherry-picks cleanly onto master **and** onto `fbf2ed61`.
...
# ground truth: coverage analysis off, scoped to the second assembly's mutants (~30 s)
...
# the bug: identical, with coverage analysis on (the default, ~1 min)
dotnet run --project ../../src/Stryker.CLI/Stryker.CLI -c Release -- --solution MicrosoftTestPlatform.slnx --test-runner mtp
```

On master: **7 × No coverage** with analysis on, against all 7 tested with it off — the same seven mutants,
each called by name from the added test (**C1**). On `fbf2ed61` — check that commit out before the
cherry-pick — the same two commands additionally vary between identical runs: three runs gave three
different verdict sets, with no warning in any of them (**C2**). Full numbers in
